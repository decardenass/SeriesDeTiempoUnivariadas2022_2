...
ts_info(banco.ts.xts)
frequency(banco.ts.xts)
...
lambda
#timetk::box_cox_vec(banco.ts,lambda = lambda,silent = F)
timetk::box_cox_vec(banco.ts.xts,lambda = lambda,silent = F)
par(mfrow=c(1,1))
plot(timetk::box_cox_vec(banco.ts.xts,lambda = lambda,silent = F),main = "Precio al cierre de acciones Bancolombia con varianza marginal ajustada", xlab = "Tiempo", ylab = "Precio")

par(mfrow=c(1,2))
plot(banco.ts.xts, main = "Precio al cierre de acciones Bancolombia sin varianza marginal ajustada", xlab = "Tiempo", ylab = "Precio")
# lbanco.ts.xts <- log(banco.ts.xts)
# plot(lbanco.ts.xts,main = "Log de Precio al cierre de acciones Bancolombia con varianza marginal ajustada", xlab = "Tiempo", ylab = "Precio")
...
splits_banco_tbl=timetk::time_series_split(banco,date_var=Fecha,assess = 15,cumulative = TRUE)
splits_banco_tbl%>% tk_time_series_cv_plan()%>%plot_time_series_cv_plan(Fecha,rev(Último))

ets_banco<-modeltime::exp_smoothing(
  error="additive",
  trend="additive",
  season="none"
)%>%
  set_engine("ets")%>%
  fit(rev(Último) ~ Fecha,data=training(splits_banco_tbl))
 # Modeltime ----
 
modeltime_table(ets_banco) %>%
  modeltime_calibrate(testing(splits_banco_tbl))%>%
  modeltime_forecast(
    new_data = testing(splits_banco_tbl),
    actual_data = banco
  )%>%
